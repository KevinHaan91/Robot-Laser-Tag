...
/** Chat Box
 * On new message, append new div with message
 */
$(function () {
...
/**
 * global
 * @param {string} load - On load of the browser window
 */
window.addEventListener("load", function(){
  ...
  /**
   * @global
   * Check for now damage from robot and update health bar
   */
 ...
 buttonDownLeft.addEventListener('mousedown', function() {
  ...
  /**
   * Set up new camera socket connection
   * @param {string} robot_ip - IP address/hostname to connect to
   * @param {string} webcam_port - Port number to connect to
   * @ret {string} cam_socket - Camera Socket 
   */
  ...
  if (robot_ip == "monmodenic.student.iastate.edu")
	  ...
	  document.getElementById('video').style.transform='scaleY(1)';
  /**
   * On conneciton from the robot camera, tell user
   * @param {string} socket - Camera socket
   */  
  cam_socket.on("connection", function(socket){
    console.log("Connected to camera");
  });
  /** Rewrite camera source with new image form robot
   * @param {string} data - base64 encoded image from robot camera
   */
